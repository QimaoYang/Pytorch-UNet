...
class pascalVOCLoader(data.Dataset):
    ...
    def __init__(
        ...
        if not self.test_mode:
            for data_kind in ["train", "val", "trainval"]:
                path = pjoin(self.root, "ImageSets/Segmentation", data_kind + ".txt")
                file_list = tuple(open(path, "r"))
                file_list = [id_.rstrip() for id_ in file_list]
                self.files[data_kind] = file_list
    ...
    def setup_annotations(self):
        ...
        if len(pre_encoded) != expected:
            ...
            for ii in tqdm(sbd_train_list):
                ...
                cv2.imwrite(pjoin(target_path, ii + ".png"), lbl)

            for ii in tqdm(self.files["trainval"]):
                fname = ii + ".png"
                lbl_path = pjoin(self.root, "SegmentationClass", fname)
                lbl = self.encode_segmap(cv2.cvtColor(cv2.imread(lbl_path), cv2.COLOR_BGR2RGB))
